...
# Access control
OWNER_ID = int(os.getenv("OWNER_ID", "139099578") or 0)  # ваш Telegram user_id
ACCESS_REQUEST_COOLDOWN_SEC = 60  # чтобы не спамили запросами
...
class AllowedUser(Base):
    ...
    # В разных версиях БД колонка могла называться created_at или added_at.
    # Мы приводим к единому — created_at, а миграция ниже добавит/заполнит нужные колонки.
...
async def cb_access_request(cq: CallbackQuery):
    # acc_req:allow:<uid> or acc_req:deny:<uid>
    ...
    async with Session() as s:
        if action == "allow":
            ...
            await s.commit()
            await cq.answer("Запрещено ❌")
            try:
                await cq.message.bot.send_message(uid, "❌ Доступ к боту запрещён.")
            except Exception:
                pass
            await cq.message.edit_text((cq.message.text or "") + "\n\n❌ Запрещено")
            return
...
# ===================== Access DB (allowed users) =====================
# Храним список разрешённых пользователей в SQLite (таблица allowed_users).
# В БД могли остаться старые колонки: added_at vs created_at — миграция ниже это чинит.

async def ensure_allowed_users_schema():
    async with engine.begin() as conn:
        # 1) Базовая таблица (новая версия) — created_at
        ...
        # 2) Проверяем фактические колонки
        cols = await conn.exec_driver_sql("PRAGMA table_info(allowed_users)")
        cols = cols.fetchall()
        col_names = {c[1] for c in cols}

        # Если была старая колонка added_at — добавим created_at и скопируем данные
        if "added_at" in col_names and "created_at" not in col_names:
            await conn.exec_driver_sql(
                "ALTER TABLE allowed_users ADD COLUMN created_at DATETIME"
            )
            await conn.exec_driver_sql(
                "UPDATE allowed_users SET created_at = COALESCE(created_at, added_at, CURRENT_TIMESTAMP)"
            )

        # Если, наоборот, есть created_at, но нет added_at — добавим added_at для совместимости
        if "created_at" in col_names and "added_at" not in col_names:
            await conn.exec_driver_sql(
                "ALTER TABLE allowed_users ADD COLUMN added_at DATETIME"
            )
            await conn.exec_driver_sql(
                "UPDATE allowed_users SET added_at = COALESCE(added_at, created_at, CURRENT_TIMESTAMP)"
            )

        # Если обе есть — подравняем пустые значения
        if "created_at" in col_names and "added_at" in col_names:
            await conn.exec_driver_sql(
                "UPDATE allowed_users SET created_at = COALESCE(created_at, added_at, CURRENT_TIMESTAMP)"
            )
            await conn.exec_driver_sql(
...
async def is_allowed_user(user_id: int) -> bool:
    if user_id == OWNER_ID:
        return True
    async with Session() as s:
        found = await s.scalar(select(AllowedUser).where(AllowedUser.user_id == user_id))
        return bool(found)


# анти-спам по запросам доступа

async def request_access_from_owner(message: Message):
    uid = message.from_user.id
    now = datetime.utcnow().timestamp()
    last = _last_access_req.get(uid, 0)
    if now - last < ACCESS_REQUEST_COOLDOWN_SEC:
        return
    _last_access_req[uid] = now
    ...
    name = safe_text(message.from_user.full_name)
    username = message.from_user.username
    txt = f"🔐 Запрос доступа к боту\nID: {uid}\nИмя: {name}"
    if username:
        txt += f"\nЮзернейм: @{username}"
    ...
    try:
        ...
        # если владелец ещё не писал боту в личку — Telegram может не дать отправить сообщение
        pass

_last_access_req: dict[int, float] = {}
import os
import asyncio
from datetime import date, datetime, timedelta
from decimal import Decimal
