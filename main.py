...
print("ADMIN_USER_IDS:", ADMIN_USER_IDS, flush=True)
...
def state_is_wizard(state_str: str | None) -> bool:
    if not state_str:
        return False
    return (
        state_str.startswith("SaleWizard:")
        or state_str.startswith("IncomeWizard:")
        or state_str.startswith("DebtorWizard:")
    )


def parse_cb(data: str, expected_min_parts: int) -> list[str] | None:
    """
    Безопасно парсим callback_data, чтобы не было split-crash.
    Возвращает list частей или None.
    """
    if not data:
        return None
    parts = data.split(":")
...
async def get_name_by_id(model, obj_id: int) -> str:
...
async def menu_anywhere(message: Message, state: FSMContext):
    ...
    # Защита: админ-кнопки не должны ломать мастера
    if state_is_wizard(cur) and text in {"➕ Добавить склад", "➕ Добавить товар", "➕ Добавить банк"}:
        return await message.answer(
            "Добавление делай через кнопку ➕ под сообщением выбора (в мастере), либо нажми ❌ Отмена и добавь в справочнике.",
...
async def show_money(message: Message):
    ...
    bank_lines: list[tuple[str, Decimal]] = []
    ip_lines: list[tuple[str, Decimal]] = []
...
async def list_sales(message: Message):
    ...
    await message.answer("Чтобы управлять: напиши `продажа #ID` например: `продажа #12`", reply_markup=main_menu_kb())
...
async def sale_by_id(message: Message):
    try:
        ...
        return await message.answer("Формат: продажа #12", reply_markup=main_menu_kb())
...
async def list_incomes(message: Message):
    ...
    await message.answer("\n".join(lines), parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu_kb())
    await message.answer("Чтобы посмотреть: напиши `приход #ID` например: `приход #7`", reply_markup=main_menu_kb())
...
async def inc_by_id(message: Message):
    try:
        ...
        return await message.answer("Формат: приход #7", reply_markup=main_menu_kb())
...
async def list_debtors(message: Message):
    ...
    await message.answer("\n".join(lines), parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu_kb())
    await message.answer("Чтобы управлять: напиши `должник #ID` например: `должник #3`", reply_markup=main_menu_kb())
...
async def debtor_by_id(message: Message):
    try:
        ...
        return await message.answer("Формат: должник #3", reply_markup=main_menu_kb())
...
async def sale_prompt(message: Message, state: FSMContext):
    ...
    if step == "doc_date":
        ...
        await message.answer("Можно отменить:", reply_markup=cancel_only_kb())
        return
...
async def cal_sale_handler(cq: CallbackQuery, state: FSMContext):
    parts = parse_cb(cq.data, 4)
    if not parts:
        return await cq.answer("Ошибка календаря", show_alert=True)

    _, scope, action, payload = parts[0], parts[1], parts[2], ":".join(parts[3:])
    ...
    if action == "pick":
        try:
            ...
            return await cq.answer("Ошибка даты", show_alert=True)
...
async def sale_nav_handler(cq: CallbackQuery, state: FSMContext):
    parts = parse_cb(cq.data, 3)
    if not parts:
        return await cq.answer("Ошибка кнопки", show_alert=True)
    _, field, action = parts[0], parts[1], parts[2]
...
async def sale_choose_wh(cq: CallbackQuery, state: FSMContext):
    ...
    if action == "add_new":
        ...
        await cq.message.answer("Напиши название нового склада:", reply_markup=cancel_only_kb())
        return await cq.answer()
...
async def sale_add_warehouse_inline(message: Message, state: FSMContext):
    name = safe_text(message.text)
    if not name:
        return await message.answer("Пусто. Напиши название склада:", reply_markup=cancel_only_kb())
...
async def sale_choose_pr(cq: CallbackQuery, state: FSMContext):
    ...
    if action == "add_new":
        ...
        await cq.message.answer("Напиши название нового товара:", reply_markup=cancel_only_kb())
        return await cq.answer()
...
async def sale_add_product_inline(message: Message, state: FSMContext):
    name = safe_text(message.text)
    if not name:
        return await message.answer("Пусто. Напиши название товара:", reply_markup=cancel_only_kb())
...
async def sale_qty(message: Message, state: FSMContext):
    try:
        ...
        return await message.answer("Ошибка. Введи число > 0, например 10 или 10.5", reply_markup=cancel_only_kb())
...
async def sale_price(message: Message, state: FSMContext):
    try:
        ...
        return await message.answer("Ошибка. Введи число, например 250 или 250.5", reply_markup=cancel_only_kb())
...
async def sale_delivery(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer("Ошибка. Введи число, например 0 или 1500", reply_markup=cancel_only_kb())
...
async def sale_status_chosen(cq: CallbackQuery, state: FSMContext):
    parts = parse_cb(cq.data, 2)
    if not parts:
        return await cq.answer("Ошибка статуса", show_alert=True)
...
async def sale_pay_method(cq: CallbackQuery, state: FSMContext):
    parts = parse_cb(cq.data, 2)
    if not parts:
        return await cq.answer("Ошибка оплаты", show_alert=True)

    method = parts[1]  # cash/noncash
...
async def sale_account_type_pick(cq: CallbackQuery, state: FSMContext):
    parts = parse_cb(cq.data, 2)
    if not parts:
        return await cq.answer("Ошибка счёта", show_alert=True)

    acc = parts[1]  # cash/bank/ip
...
async def sale_bank_pick(cq: CallbackQuery, state: FSMContext):
    ...
    if action == "add_new":
        ...
        await cq.message.answer("Напиши название нового банка (для Банка/ИП):", reply_markup=cancel_only_kb())
        return await cq.answer()
...
async def sale_add_bank_inline(message: Message, state: FSMContext):
    name = safe_text(message.text)
    if not name:
        return await message.answer("Пусто. Напиши название банка:", reply_markup=cancel_only_kb())
...
async def build_sale_summary(data: dict) -> str:
    ...
    pr_id = int(data["product_id"])
    wh_name = await get_name_by_id(Warehouse, wh_id)
    pr_name = await get_name_by_id(Product, pr_id)

    acc_key = data.get("account_type")
    acc = {"cash": "Наличные", "bank": "Банк", "ip": "Счёт ИП"}.get(acc_key, "-")

    bank_txt = "-"
    if acc_key in ("bank", "ip"):
        b_id = data.get("bank_id")
        if b_id:
            bank_txt = await get_name_by_id(Bank, int(b_id))
        else:
            bank_txt = "-"
...
async def sale_confirm(cq: CallbackQuery, state: FSMContext):
    parts = parse_cb(cq.data, 2)
    if not parts:
        return await cq.answer("Ошибка подтверждения", show_alert=True)
...
async def income_prompt(message: Message, state: FSMContext):
    ...
    if step == "confirm":
        data = await state.get_data()
        await message.answer(await build_income_summary(data) + "\n\nПодтвердить?",
                             parse_mode=ParseMode.MARKDOWN,
...
async def build_income_summary(data: dict) -> str:
    qty = Decimal(data["qty"])
    price = Decimal(data["price"])
    total = qty * price
    delivery = Decimal(data.get("delivery", "0"))
...
async def cal_deb_handler(cq: CallbackQuery, state: FSMContext):
    ...
    if action == "pick":
        ...
        await cq.message.answer("Можно отменить:", reply_markup=cancel_only_kb())
        return await cq.answer()
...
async def deb_qty(message: Message, state: FSMContext):
    try:
        ...
        return await message.answer("Ошибка. Введи число, например 10 или 10.5", reply_markup=cancel_only_kb())
    await state.update_data(qty=str(q))
    await state.set_state(DebtorWizard.price)
...
async def deb_price(message: Message, state: FSMContext):
    try:
        ...
        return await message.answer("Ошибка. Введи число, например 250", reply_markup=cancel_only_kb())
    await state.update_data(price=str(p))
    await state.set_state(DebtorWizard.delivery)
...
async def deb_delivery(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer("Ошибка. Введи число, например 0", reply_markup=cancel_only_kb())
    await state.update_data(delivery=str(d))
    await state.set_state(DebtorWizard.confirm)
    data = await state.get_data()
